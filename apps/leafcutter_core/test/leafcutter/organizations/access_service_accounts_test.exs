  ...
  describe "revoke_role/1" do
    test "is idempotent and revokes only the requested scope" do
      ...
      assert %ServiceAccountRoleAssignment{} =
               Repo.one(
                 from(assignment in ServiceAccountRoleAssignment,
                   where:
                     assignment.service_account_id == ^service_account.id and
                       assignment.role_id == ^role.id and
                       is_nil(assignment.environment_id)
                 )
               )
