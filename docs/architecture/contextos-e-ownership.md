...
As APIs públicas `Catalog.Contracts.compile/1` e `validate/2` estão materializadas. Compilação, normalização de erros e política de referência pertencem ao Catalog. PackageVersion, EnvironmentDeployment e o resolver de Run consomem a executabilidade em suas boundaries ratificadas. Schema e validator não mudam de owner.
