...
| Catalog | PARCIALMENTE MATERIALIZADO | Connector, ConnectorVersion, Operation metadata, Contract, ContractVersion executável com schema/JSV, Package, PackageVersion, endpoints com rejeição de legado e manifest digest | availability/deprecation metadata |
...
As APIs públicas `Catalog.Contracts.compile/1` e `validate/2` estão materializadas.
Compilação, normalização de erros e política de referência pertencem ao Catalog.
PackageVersion, EnvironmentDeployment e o resolver de Run consomem a executabilidade em suas
boundaries ratificadas. Schema e validator não mudam de owner.

Manifest/binding pertencem a `leafcutter_connectors`; inventory, release closure e resolução
compilada pertencem a `leafcutter_runtime`. O Catalog possui somente a projeção relacional e
o digest autoritativo necessários para essa composição.
